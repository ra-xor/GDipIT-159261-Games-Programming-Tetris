...
public abstract class GameEngine implements KeyListener, MouseListener, MouseMotionListener {
	...
	public void setupWindow(int width, int height) {
		...
		// Resize the window (insets are just the boarders that the Operating System
		// puts on the board)
	...
	public Image makeImageTransparent(Image sourceImage, float opacity) {

		// Convert to a buffered image
		BufferedImage buffered = (BufferedImage) sourceImage;

		BufferedImage transparentImage = new BufferedImage(buffered.getWidth(), buffered.getHeight(),
				BufferedImage.TYPE_INT_ARGB);

		mGraphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, opacity));
		mGraphics.drawImage(sourceImage, 0, 0, null);
		mGraphics.dispose();
	...
	// Method to get FontMetrics from the current graphics context
	public FontMetrics getFontMetrics() {
		if (mGraphics == null) {
			// This case should ideally not be reached if called from within paintComponent
			// flow.
			System.err.println("Error: getFontMetrics() called when mGraphics was null. Attempting fallback.");
			// Fallback: Try to get FontMetrics from the panel with a default font.
			if (mPanel != null) {
				Font defaultFont = mPanel.getFont();
				if (defaultFont == null) {
					defaultFont = new Font("SansSerif", Font.PLAIN, 12);
				}
				try {
					// Check if panel is displayable before calling getFontMetrics
					if (mPanel.isDisplayable()) {
						return mPanel.getFontMetrics(defaultFont);
					} else {
						System.err.println("mPanel not displayable. Using BufferedImage for FontMetrics.");
					}
				} catch (Exception e) {
					System.err.println(
							"Could not get FontMetrics from mPanel: " + e.getMessage() + ". Using BufferedImage.");
				}
			}
			// Final fallback: create a dummy graphics context. This is inefficient but
			// better than returning null.
			BufferedImage bi = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
			Graphics g = bi.getGraphics();
			Font defaultFontForBI = new Font("SansSerif", Font.PLAIN, 12); // Ensure font for BI graphics
			FontMetrics fm = g.getFontMetrics(defaultFontForBI);
			g.dispose();
			return fm;
		}
